...
public class Main {
    ...
    public static void main(String[] args) {
     Dog dog1 = new Dog();
        ...
        animal1.makeSound();// 결과 값 : 개가 짖습니다 ,동물이 소리를 냅니다.
...
//        animal2.fetch();  -> 오류 발생
        ...
        System.out.println("↑업 캐스팅 관련 수업");
        System.out.println("↓ 다운캐스팅 관련 수업");
        /*
            업캐스팅의 경우에는 암시적(형변환없이도 자동수행)으로 이루어집니다. 그래서 A a = new B(); 보고 대충 업캐스팅됐구나 생각하는 반면
        ...
        dog2.fetch();
        animal2.makeSound();
        ((Dog) animal2).fetch(); // Dog dog2 = (Dog)animal2; 를 만듬으로써 자동완성으로 다운캐스팅이 이루어짐
...
//        dog3.makeSound();
        /*
            Dog dog3 = (Dog)animal3;
            dog3.makeSound();
            코드에는 오류가 발생하지 않는데  출력했을때 오류가 생긴다는점에서 차이가 있음

            이유 :
                애초에 animal3를 생성할 때 new Animal();로 생성했기 때문에 다운캐스팅이 불가능하다.
                즉 , 다운캐스팅의 전제 조건은 A a = new B();에서 출발한다는 점을 확인할 수 있음.

                그래서 지금 경우의 수가 animal**이라는 객체가 있을 때 걔가 Animal() 생성자로 생성됐는지
                Dog() 생성자로 생성됐는지에 따라 우리 프로그램이 오류가 날 수도 있고 안날수도 있는 양자역학 상황이라는 점이다.
         */

        Animal animal4 = new Dog();
        System.out.println("animal4 검증파트");
        if (animal4 instanceof Dog) {   // 얘가 true라면 animal4 객체는 Animal animal4 = new Dog()로 생성된다
            Dog dog4 = (Dog) animal4;   //그래서 다운캐스팅이 가능하다

            dog4.makeSound();
            dog4.fetch();           //얘를 실행시켜도 오류가 발생하지 않는다
